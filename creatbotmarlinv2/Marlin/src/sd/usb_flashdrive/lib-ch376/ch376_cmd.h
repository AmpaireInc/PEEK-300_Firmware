...
#define CMD20_SET_SDO_INT     0x0B /* SPI接口方式: 设置SPI的SDO引脚的中断方式 */
                                   /* 输入: 数据16H, 中断方式 */
                                   /* 输入: 10H=禁止SDO引脚用于中断输出,在SCS片选无效时三态输出禁止,*/
                                   /*     90H=SDO引脚在SCS片选无效时兼做中断请求输出 */
...
#define _NO_CH375_COMPATIBLE_ /*禁用CH375兼容*/
...
typedef union _CH376_CMD_DATA {
  ...
  struct {
    ...
    UINT32 mSectorSize;  /* 返回: 每个扇区的大小(总长度32位,高字节在前) */
  ...
  struct {
    UINT8 mDiskData[64]; /* 返回: 扇区的数据 */
  } DiskReadData;        /* CMD5H_DISK_READ: 从USB存储器读出的数据块 */
...
/* 主机文件模式下的文件系统变量的地址 */
  ...
  #define VAR_UDISK_LUN      0x34 /* USB存储设备的逻辑单元号 */
                                  /*     位7~位4, USB存储设备的当前逻辑单元号 */
                                  /*         CH376初始化USB存储设备后,默认是访问0#逻辑单元 */
