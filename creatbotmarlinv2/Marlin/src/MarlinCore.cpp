 ...
 * 注册码是否正确
 */
bool isKeyPass() { return serialkey.checkReg(); }
...
bool isLimited() {
    ...
    return ELAPSED(stats.printTime, TOTAL_TIME_LIMIT) ? !isKeyPass() : false;
  #else
    return !isKeyPass();
  #endif
}
...
void startOrResumeJob() {
  ...
  void powerOffOnCooldown() {
    ...
    TERN_(SDSUPPORT, card.abortFilePrintNow(TERN_(SD_RESORT, true)));  // 中止存储器打印任务
...
void setup() {
  ...
  TERN_(SUPPORT_REG_KEY, SETUP_RUN(serialkey.initSequence()));
