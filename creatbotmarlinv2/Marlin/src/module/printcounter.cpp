...
void PrintCounter::checkRegKey() {
  bool        needLimit     = ELAPSED(data.printTime, TOTAL_TIME_LIMIT) ? !isKeyPass() : false;
  static bool lastNeedLimit = needLimit;
  if (needLimit != lastNeedLimit) {
    lastNeedLimit = needLimit;
    saveStats();
    if (needLimit) ExtUI::onSerialKeyTimeReach();
  }
}
  #endif
...
void PrintCounter::tick() {
  ...
  if (ELAPSED(now, update_next)) {
    ...
    TERN_(SUPPORT_REG_KEY, checkRegKey());
