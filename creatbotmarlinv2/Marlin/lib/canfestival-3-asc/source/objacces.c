...
#  ifdef CANOPEN_BIG_ENDIAN
  if(endianize && *pDataType > CanOpen_boolean && !(
         *pDataType >= CanOpen_visible_string &&
         *pDataType <= CanOpen_domain)) {
    /* data must be transmited with low byte first */
    UNS8 i, j = 0;
    MSG_WAR(CanOpen_boolean, "data type ", *pDataType);
    MSG_WAR(CanOpen_visible_string, "data type ", *pDataType);
    for ( i = szData ; i > 0 ; i--) {
      MSG_WAR(i," ", j);
      ((UNS8*)pDestData)[j++] =
...
#  endif
  ...
  if(*pDataType != CanOpen_visible_string)
      *pExpectedSize = szData;
  else {
  ...
  if( *pExpectedSize == 0 || *pExpectedSize <= szData )
    {
      ...
      if(endianize && dataType > CanOpen_boolean && !(
            dataType >= CanOpen_visible_string && 
            dataType <= CanOpen_domain))
        {
      ...
      * */
      ...
      if(dataType == CanOpen_visible_string && *pExpectedSize < szData)
